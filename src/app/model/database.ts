...
export class PasswordEncryptingJsonSerializationContext implements JsonSerializationContext {
    static readonly prefix = 'PrefixForDetectingValidPassword_';

    public constructor(private password: string) {
    }
    encryptData(text: string): string {
        return AES.encrypt(PasswordEncryptingJsonSerializationContext.prefix + text, this.password).toString();
    }
    decryptData(text: string): string {
        try {
            const decrypted = AES.decrypt(text, this.password).toString(CryptoEnc.Utf8);
            if (!decrypted.startsWith(PasswordEncryptingJsonSerializationContext.prefix)) {
                throw new Error('Password maybe wrong?');
            }
            return decrypted.substring(PasswordEncryptingJsonSerializationContext.prefix.length);
        }
        catch (e) {
...
export class TimeTrackingPeriod implements JsonSerializable {
    ...
    projectHours: Map<number, ProjectSettings> = new Map<number, ProjectSettings>();
    ...
    toJSON(): any {
        return {
            ...
            projectHours: Object.fromEntries(Array.from(this.projectHours.entries()).map(kvp => [kvp[0], kvp[1].toJSON()]))
        }
    }
    fromJSON(json: any): void {
        ...
        this.projectHours = new Map<number, ProjectSettings>(
            Object.entries(json).map(kvp => [Number(kvp[0]), new ProjectSettings().fromJSON(kvp[1])])
        )
    }
...
export class Database implements JsonSerializable {
    ...
    async writeToFile(handle: FileSystemFileHandle, context: JsonSerializationContext) {
        const json = JSON.stringify(this.toJSON(context));
